...
export default function TabletNav() {
  // ✨ Hook directly into the router's location state
        ...
        <div className="flex flex-col flex-1 w-full relative overflow-visible">
          <div className="flex flex-col gap-2 w-full relative">
            {tabsTablet.map(({ id, Icon }) => {
              // ✨ BUG FIX: Use .startsWith to keep the link highlighted on nested routes
          ...
          <div className="mt-auto w-full">
            {/* Using an IIFE pattern to calculate account's active state cleanly */}
            {(() => {
              ...
              return (
                ...
                >
                  ...
                  {/* Icon Content */}
                  <div className="relative z-30 w-10 h-10 flex items-center justify-center transition-transform duration-150 active:scale-90">
                    <AccountIcon className={`transition-all duration-500 w-9 h-9 ${isAccountActive ? 'text-MobileNav' : 'text-white'}`} />
                  </div>
                </Link>
              );
            })()}
